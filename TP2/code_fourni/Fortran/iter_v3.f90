...
!   with Rayleigh-Ritz projection + block approach (v2) + deflation (v3) (TO BE DEVELOPPED)
!
!--------------------------------------------------------------------------------------------------------------------
!
!! subspace_iter_v3
...
! using the subspace iteration method with Rayleigh-Ritz projection + block approach (v2) + deflation (v3)
!
! ..Input arguments
!    n : integer, size of the matrix A
!    m : integer, size of the searched subspace
!    a : double precision matrix
!    p : number of products Y = A^p . V
!
...
!-------------------------------------------------------------------------------------------------------------------
  subroutine subspace_iter_v3(n, m, a, p, percentage, maxit, eps, v, w, n_ev, acc_ev, it_ev, ierr)
    implicit none
    !! the subspace dimensions
    integer,          intent(in)                     :: n, m
    !! the target matrix                              
    double precision, dimension(n, n), intent(in)    :: a
    !! number of products Y = A^p . V
    integer,          intent(in)                     :: p
    ...
    do while((eig_sum .lt. p_trace) .and. (n_ev .lt. m) .and. (k .lt. maxit))
      ...
      !! A. Compute  Y(:,n_ev+1:m) = A^p * V_nc 
      call dgemm('n','n', n, m-n_ev, n, done, a, n, v(1,n_ev+1), n, dzero, y(1,n_ev+1), n)
      do i = 2, (p)
        v(:, n_ev+1:m) = y(:, n_ev+1:m)
        call dgemm('n','n', n, m-n_ev, n, done, a, n, v(1,n_ev+1), n, dzero, y(1,n_ev+1), n) 
      end do
      !! ensure { Y = A^p * V_nc }
      ...
      call gram_schmidt_block(y, n, m, n_ev, v)
      ...
      call dgemm('n','n', n, m-n_ev, n, done, a, n, v(1,n_ev+1), n, dzero, y(1,n_ev+1), n)
      !!     H = V_nc'*Y
      call dgemm('t', 'n', m-n_ev, m-n_ev, n, done, v(1,n_ev+1), n, y(1,n_ev+1), n, dzero, h, m) 
      !!   2. Spectral decomposition
      call dsyev('v', 'u', m-n_ev, h, m, w_aux, work, lwork, ierr) 
      if( ierr .ne.0 )then
      ...
      do i = 1, m-n_ev
        t(i+n_ev) = w_aux(m-n_ev-i+1)
        x(:, i+n_ev)  = h(:, m-n_ev-i+1)
      end do

      !!   3. V = VX
      y(:, n_ev+1:m) = v(:, n_ev+1:m)
      call dgemm('n', 'n', n, m-n_ev, m-n_ev, done, y(1,n_ev+1), n, x(1,n_ev+1), m, dzero, v(1,n_ev+1), n)
...
999 continue
    ...
    return
  end subroutine subspace_iter_v3
